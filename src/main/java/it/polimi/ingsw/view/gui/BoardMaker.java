...
/**
 * this class offers static methods to the panels of the GUI to draw the
 * Boardproxy on the window of the application.
 *
 * @author Elia Ravella
 */
public class BoardMaker {
    ...
    private static final int IMAGE_WIDTH = 810;
    private static final int IMAGE_HEIGHT = 810;
    private static final int CELL_LENGTH = 162;

    /**
     * draws the towers on the board given. all the measures are in pixels the board
     * is assumed a perfect square
     *
     ...
     * @author Elia Ravella
     */
    public static void drawElements(Graphics g, BoardProxy board, int offset, int width, int interstitialWidth,
            Component obs) {
        Image first;
        Image second;
        Image third;
            ...
            for (int col = 0; col < board.getBoardScheme()[row].length; col++) {
                switch (board.getBoardScheme()[row][col]) {
                    case GROUND:
                        break;
                    case FIRST_FLOOR:
                        g.drawImage(first, offset + (width + interstitialWidth) * row,
                                offset + (width + interstitialWidth) * col, width, width, obs);
                        break;
                    case SECOND_FLOOR:
                        g.drawImage(second, offset + (width + interstitialWidth) * row,
                                offset + (width + interstitialWidth) * col, width, width, obs);
                        break;
                    case THIRD_FLOOR:
                        g.drawImage(third, offset + (width + interstitialWidth) * row,
                                offset + (width + interstitialWidth) * col, width, width, obs);
                        break;
                    case DOME:
                        g.drawImage(dome, offset + (width + interstitialWidth) * row,
                                offset + (width + interstitialWidth) * col, width, width, obs);
                        break;
                }
            }
        ...
        for (int i = 0; i < board.getWorkers().keySet().size(); i++) {
            String player = board.getWorkers().keySet().toArray()[i].toString().substring(0,
            ...
            g.drawImage(godImage, actualCoordinates.x, actualCoordinates.y, width, width, obs);
        }
    }
    ...
    public static Pair map(int x, int y) {
        return new Pair((x / CELL_LENGTH) % (IMAGE_WIDTH / CELL_LENGTH),
    ...
    private static Pair deMap(Pair pair) {
        return new Pair(pair.x * CELL_LENGTH, pair.y * CELL_LENGTH);
    }
}
